...
# python initRun.py "neighborsim" "Epsilon"
 python initRun.py "structuresim" "Epsilon"
	...
	do
		...
		echo $displayEpsilon
		#ln -sf "result/"$data"_"$T"_"$D"_"$displayEpsilon".pathsim "result/"$data"_"$T"_"$D"_"$displayEpsilon".topK.pathsim"
		#echo "ln"
		#python calNeighborSimScore.py $data $T $D $displayEpsilon "pathsim" $topK "Epsilon" $max
		...
		python calStructureSimScore.py $data $T $D $displayEpsilon "pathvec_2" $topK "Epsilon" $max
	done
