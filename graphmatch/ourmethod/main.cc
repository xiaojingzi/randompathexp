...
void init(char* data, int t, int d, double epsilon){
  ...
  sprintf(similarPathfile,"result/%s.pathsim",dataset);
  sprintf(similarStructurefile,"result/%s.pathvec",dataset);
...
void loadLink(){
  ...
  while(!feof(fp)){
    ...
    GWeightedDegree[a].second += c;
    
    G[b].push_back(make_pair(a,c));
    GWeightedDegree[b].second += c;
...
void generateRandomPath(){
  ...
  accumated_degee.push_back(make_pair(GWeightedDegree[0].first, GWeightedDegree[1].second));
  

  int neighborlen = G[0].size();
    if (neighborlen > 0){
      sort(G[0].begin(), G[0].end(), CmpByValue());
      GAccumulatedNeighborVector[0].push_back(make_pair(G[0][0].first, G[0][0].second / GWeightedDegree[0].second));
      if(neighborlen > 1){
        for(unsigned int j = 1 ; j < neighborlen ; j++) {
          GAccumulatedNeighborVector[0].push_back( make_pair(G[0][j].first, G[0][j].second/ GWeightedDegree[0].second + GAccumulatedNeighborVector[0][j-1].second));
        }
      }
    }
